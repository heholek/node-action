  ...
  scan: function(src, callback) {
    fs.readFile(src, function(err, data) {
      ...
      var canvas = new Canvas.Canvas(1, 1),
        ...
        addMerge = function(from, to) {
          ...
          while (len--) {
            ...
            rlen = region.length;
            while (rlen--) {
              if (region[rlen] == from) fromIndex = len;
              if (region[rlen] == to) toIndex = len;
            }
          }
          ...
          if (fromIndex != -1 && toIndex != -1 && fromIndex != toIndex) {
            mergeRegions[fromIndex] = mergeRegions[fromIndex].concat(
              mergeRegions[toIndex],
            );
            mergeRegions = [].concat(
              mergeRegions.slice(0, toIndex),
              mergeRegions.slice(toIndex + 1),
            );
          }
        },
        totalPixels,
      ...
      for (var i = 0, u = 1; i < length; i += 4, u++) {
        ...
        if (classifySkin(r, g, b)) {
          ...
          if (!checker) {
            ...
            if (!detectedRegions[region]) detectedRegions[region] = [];
      ...
      while (length--) {
        ...
        while (rlen--) {
          index = region[rlen];
          detRegions[length] = detRegions[length].concat(
      ...
      for (var i = 0; i < length; i++)
        if (detRegions[i].length > 30) skinRegions.push(detRegions[i]);
      length = skinRegions.length;
      if (length < 3) return callback && callback(false);
      (function() {
        var sorted = false,
          temp;
        while (!sorted) {
      ...
      while (length--) totalSkin += skinRegions[length].length;
      if ((totalSkin / totalPixels) * 100 < 15)
        return callback && callback(false);
      if (
        (skinRegions[0].length / totalSkin) * 100 < 35 &&
      ...
      if ((skinRegions[0].length / totalSkin) * 100 < 45)
        return callback && callback(false);
      if (skinRegions.length > 60) return callback && callback(false);
